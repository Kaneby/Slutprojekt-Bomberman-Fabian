function Explosion(startX, startY, whichPlayer, teamColour, explosionRange) {
    ...
    this.explosionRender = function (player) {
        ...
        for (var i = 0; i <= this.downExp; i++) {
            //ner sprängning
            if ((this.xPos / 50) % 2 == 0) {
                ctx.fillRect(this.xPos, this.yPos + 50 * i, 50, 50);
            }
        }
    ...
    this.kill = function (player) {
        ...
        for (var i = 1; i < players.length; i++) {
            ...
            if (this.xPos - (player.range * 50) <= players[i].xPos &&
                players[i].xPos <= this.xPos + (player.range * 50) &&
                this.yPos == players[i].yPos ||
                this.yPos - (player.range * 50) <= players[i].yPos &&
                players[i].yPos <= this.yPos + (player.range * 50) &&
                this.xPos == players[i].xPos) {

                console.log(players[i].yPos, this.yPos + player.range * 50, players[i].xPos, this.xPos + player.range * 50);
